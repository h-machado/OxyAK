...
var options = stdio.getopt({
	'configuration': {    key: 'c', args: 1, description: 'Path to the configuration json file (defaults to ./oxyak.json)'},
...
var configFilename = (options.configuration)?process.cwd() + "/" + options.configuration:"./oxyak.json";
if (!fs.existsSync(configFilename, fs.F_OK)) {
	configFilename = (options.configuration)?options.configuration:"./oxyak.json";;
}
...
|_|
*/
var oxyak = function(_config, _options) {
...
var l = new oxyak(config, options);
/*
  ____                                          _
...
if ((options.failoverMonkey) && (options.failoverMonkey.constructor === Array) && ((options.failoverMonkey.length > 0))){
	...
	for (var idx = 0; idx < options.failoverMonkey.length; idx++) {
		...
		configuration[u.href]['runtime'] = new oxyak(setup, options);
	}
